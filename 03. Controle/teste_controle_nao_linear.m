...
% hold on
subplot(5,1,5)
plot_angular_position_ff(Thr_lin.signals.values(:,5), Thr_nonlin.signals.values(:,5), Thr_nonlin_pre_comp.signals.values(:,5), Thd_path.signals.values(:,5), Thr_lin.time, 5)
% grid('on')
% xlabel('Tempo (s)')
% ylabel('Posição(º)')
...
function plot_angular_position_ff(thr_lin, thr_nonlin, thr_nonlin_pre_comp, thd, tout, joint_number)
    plot(tout,thr_lin,'b',tout,thr_nonlin,'r',tout,thr_nonlin_pre_comp,'g',tout,thd,strcat('--','k'));
    ...
    legend(strcat('Lin $\theta_{',int2str(joint_number),'r}$'), strcat( ...
         ...
         'Non lin FF $\theta_{', int2str(joint_number),'r}$'), strcat( ...
         '$\theta^d_', int2str(joint_number),'$'), 'Interpreter', 'latex','FontSize',8);
    set(gca,'FontSize',8)
end